...
function curryDefinition(
    ...
    if (filteredOverloads.length === 0)
        filteredOverloads = overloads.filter(
            o => o.params.length > 0 && o.params.length <= arity + 1 && o.params[o.params.length - 1].startsWith('...'),
        );
    if (filteredOverloads.length === 0)
    ...
    interfaceTypeParams.unshift(
        ...overload.typeParams.filter(
            tp =>
                ...
                interfaceTypeParams.some(tp2 => !!tp2.extends && new RegExp(`^keyof ${tp.name}\\b`).test(tp2.extends)),
        ),
    );
